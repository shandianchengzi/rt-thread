...
typedef struct {
    uint8_t  clk_src;       //ADCADC_CLKSRC_HRCADC_CLKSRC_XTAL...
    uint8_t  samplAvg;      //ADCADC

    uint8_t  EOC_IEn;       //EOC     ADC_SEQ0ADC_SEQ1ADC_SEQ2ADC_SEQ3
    uint8_t  HalfIEn;       //FIFOADC_SEQ0ADC_SEQ1ADC_SEQ2ADC_SEQ3
} ADC_InitStructure;

typedef struct {
    uint16_t channels;      //ADCADC_CH0ADC_CH1... ... ADC_CH7
    uint8_t  trig_src;      //ADCADC_TRIGGER_SWADC_TRIGGER_TIMER2ADC_TRIGGER_TIMER3... ...
    uint16_t conv_cnt;      //ADC1--256
    uint8_t  samp_tim;      //ADCADC_SAMPLE_1CLOCKADC_SAMPLE_2CLOCKADC_SAMPLE_4CLOCKADC_SAMPLE_8CLOCK
} ADC_SEQ_InitStructure;

typedef struct {
    uint16_t UpperLimit;    //
    uint16_t UpperLimitIEn; //ADCUpperLimit
    uint16_t LowerLimit;    //
    uint16_t LowerLimitIEn; //ADCLowerLimit
...
#define ADC_IT_CMP_MAX      (1 << 4)    //COMP.MAX
#define ADC_IT_CMP_MIN      (1 << 5)    //COMP.MIN


void ADC_Init(ADC_TypeDef * ADCx, ADC_InitStructure * initStruct);      //ADC
void ADC_SEQ_Init(ADC_TypeDef * ADCx, uint32_t seq, ADC_SEQ_InitStructure * initStruct);    //ADC
void ADC_CMP_Init(ADC_TypeDef * ADCx, uint32_t seq, ADC_CMP_InitStructure * initStruct);    //ADC
void ADC_Open(ADC_TypeDef * ADCx);                          //ADCADC
void ADC_Calibrate(ADC_TypeDef * ADCx);                     //ADC
void ADC_Close(ADC_TypeDef * ADCx);                         //ADCADC
void ADC_Start(ADC_TypeDef * ADCx, uint32_t seq);           //ADC
void ADC_Stop(ADC_TypeDef * ADCx, uint32_t seq);            //ADC
